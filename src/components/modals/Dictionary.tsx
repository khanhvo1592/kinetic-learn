      ...
      <div className="bg-white rounded-[24px] border-4 border-orange-400 p-6 w-full max-w-lg shadow-[0_16px_0_0_#b75b00]">
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            placeholder="Tìm kiếm Proton, Đạo hàm..."
            className="w-full bg-slate-50 border-2 border-slate-200 focus:border-orange-400 focus:outline-none rounded-xl py-3 pl-10 pr-4 text-sm font-sans"
          />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 h-64">
          <div className="md:col-span-5 overflow-y-auto border border-slate-100 rounded-xl divide-y divide-slate-100 p-1 bg-slate-50 scroll-hide">
            {filtered.length > 0 ? (
              filtered.map((t) => (
                <button
                  key={t.word}
                  ...
                  className={`w-full text-left p-2.5 rounded-lg text-xs font-semibold font-display transition-colors ${
                    selectedTerm?.word === t.word ? 'bg-orange-100 text-orange-800' : 'hover:bg-white text-slate-700'
                  }`}
                >
                  {t.word}
          ...
          <div className="md:col-span-7 bg-orange-50/40 rounded-xl p-4 border border-orange-100 flex flex-col justify-between overflow-y-auto scroll-hide">
            {selectedTerm ? (
              <div>
