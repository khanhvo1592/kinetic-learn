...
export default function AITutor({ onClose, initialTopic }: AITutorProps) {
  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      ...
      console.warn('Không thể kết nối với AI Tutor backend:', err);
    } finally {
      setIsTyping(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-[24px] border-4 border-cyan-400 p-6 w-full max-w-md shadow-[0_16px_0_0_#00bcd4] flex flex-col h-[520px]">
        ...
        <div className="flex justify-between items-center mb-4 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-sans tracking-wide">Giải bài tập 24/7</p>
            </div>
          </div>
        ...
        {/* Message panel */}
        <div className="flex-1 overflow-y-auto mb-4 space-y-3 p-2 bg-slate-50/50 rounded-2xl border border-slate-100 scroll-hide">
          {messages.map((m) => (
            <div key={m.id} className={`flex flex-col max-w-[85%] ${m.sender === 'user' ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
              <div className={`p-3 rounded-2xl text-xs leading-relaxed font-sans ${m.sender === 'user' ? 'bg-cyan-500 text-white rounded-br-none shadow-sm' : 'bg-white text-slate-800 rounded-bl-none border border-slate-200/60 shadow-sm whitespace-pre-line'}`}>
                {m.text}
              </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-2 scroll-hide flex-shrink-0">
          {SUGGESTIONS.map((s) => (
            <button key={s} onClick={() => handleSend(s)} className="px-3 py-1.5 bg-cyan-50 text-cyan-700 rounded-full text-[10px] font-bold font-display hover:bg-cyan-100/80 active:scale-95 transition-all whitespace-nowrap">
              {s}
            </button>
          ))}
        </div>
        ...
        {/* Input box */}
        <form onSubmit={(e) => { e.preventDefault(); handleSend(inputMsg); }} className="flex gap-2 mt-2 flex-shrink-0">
          <input type="text" value={inputMsg} onChange={(e) => setInputMsg(e.target.value)} placeholder="Hỏi AI oát..." className="flex-1 bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-cyan-400 font-sans" />
          <button type="submit" className="w-11 h-11 bg-cyan-500 text-white rounded-xl flex items-center justify-center shadow-[0_4px_0_0_#00bcd4] hover:bg-cyan-600 active:translate-y-[2px] active:shadow-none transition-all flex-shrink-0">
