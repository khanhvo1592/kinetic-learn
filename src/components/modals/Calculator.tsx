...
export default function Calculator({ onClose }: CalculatorProps) {
  ...
  const calculate = () => {
    try {
      ...
      // Simple safety check before evaluation
      const safePattern = /^[0-9+\-*/(). MathPItansicqr]+$/;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-[24px] border-4 border-[#0058be] p-6 w-full max-w-sm shadow-[0_16px_0_0_#004395]">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-[#0058be] text-2xl">calculate</span>
            <h3 className="font-display font-bold text-lg text-[#0058be]">Máy tính đa năng</h3>
          </div>
        ...
        <div className="bg-slate-900 rounded-xl p-4 text-right font-mono mb-4 border-2 border-slate-700">
          ...
          <div className="text-white text-2xl font-bold overflow-x-auto truncate scroll-hide">{display}</div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          ...
          <button onClick={() => handleOp(')')} className="p-3 bg-indigo-50 text-indigo-700 font-semibold rounded-xl text-sm hover:bg-indigo-100 active:scale-95 transition-all">)</button>
          <button onClick={() => handleOp('÷')} className="p-3 bg-blue-50 text-[#0058be] font-bold rounded-xl text-sm hover:bg-blue-100 active:scale-95 transition-all">÷</button>
          ...
          <button onClick={calculate} className="p-4 bg-[#0058be] text-white font-bold rounded-xl hover:bg-[#004395] shadow-[0_4px_0_0_#003370] active:translate-y-[4px] active:shadow-none transition-all">=</button>
        </div>
      </div>
    </div>
  );
